function [z0_est, out] = imr1(w_f0, w_b0, params )
...
% Version: 10/24/2014
    ...
    kernel_F=ones(1,kernelWidthF)/kernelWidthF;
    kernel_B=ones(1,kernelWidthB)/kernelWidthB;
    u = 0:1:columns-1;
...
%                 end
               ...
               % w1_um(i) = interpolateY(w_b1, u(i)*m1_k(i)+1);   % find the backflow at u*m    
                ...
                disp(strcat('Interpolate failed at i=',num2str(i),' j=',num2str(j)))
                pause;
            end            
        end
...
%             rms_flow_error = sqrt(mean((I0z).*conj(I0z)));
        ...
        updatePlots = 1;
        if updatePlots == 1 || j == params.iterations0
            updatePlots = 1;
